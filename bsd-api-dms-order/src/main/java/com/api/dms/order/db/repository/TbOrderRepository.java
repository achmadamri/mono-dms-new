...
public interface TbOrderRepository extends JpaRepository<TbOrder, Integer> {
	...
	@Query("select t1 from TbOrder t1 join TbUserMarket t2 on t2.tbmMarketId = t1.tboMarketId and t2.tbmMarketCheck = 1 and t2.tbuId = :tbuId where t1.tboOrderNo like %:tboOrderNo% and t1.tboStatus like :tboStatus% and t1.tboStatus != 'Completed' and t1.tboCreateDate >= :startDate and t1.tboCreateDate <= :endDate")
	...
	@Query("select count(0) from TbOrder t1 join TbUserMarket t2 on t2.tbmMarketId = t1.tboMarketId and t2.tbmMarketCheck = 1 and t2.tbuId = :tbuId where t1.tboOrderNo like %:tboOrderNo% and t1.tboStatus like :tboStatus% and t1.tboStatus != 'Completed' and t1.tboCreateDate >= :startDate and t1.tboCreateDate <= :endDate")
