...
import com.api.dms.order.db.repository.TbUserMarketRepository;
...
import com.api.dms.order.util.MD5;
	...
	public PostUserAddResponseModel postUserAdd(PostUserAddRequestModel requestModel) throws Exception {
		...
		if (optTbUser.isPresent()) {
			...
			if (optTbUserNew.isPresent()) {
				...
				for (com.api.dms.order.model.user.TbUserMarket tbUserMarket_ : requestModel.getLstTbUserMarket()) {
				...
				for (com.api.dms.order.model.user.TbUserBrand tbUserBrand_ : requestModel.getLstTbUserBrand()) {
					com.api.dms.order.db.entity.TbUserBrand tbUserBrand = new com.api.dms.order.db.entity.TbUserBrand();
	...
	public PostUserEditResponseModel postUserEdit(PostUserEditRequestModel requestModel) throws Exception {
		...
		if (optTbUser.isPresent()) {
			...
			if (optTbUserExisting.isPresent()) {
				...
				List<TbUserMarket> lstTbUserMarket = tbUserMarketRepository.findAll(Example.of(exampleTbUserMarket));
				tbUserMarketRepository.deleteAll(lstTbUserMarket);
				for (com.api.dms.order.model.user.TbUserMarket tbUserMarket : requestModel.getLstTbUserMarket()) {
