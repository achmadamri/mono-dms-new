...
import { GetSkuProductRequest } from './getskuproductrequest';
import { GetSkuProductResponse } from './getskuproductresponse';
...
export class GwpService {
  util: Util = new Util();
  apiUrl = isDevMode() ? 'http://localhost:2084/gwp' : 'https://dms.id-trec.com/2084/gwp';
  ...
  getSkuProduct(getSkuProductRequest: GetSkuProductRequest): Observable<GetSkuProductResponse> {
    const headers = new HttpHeaders()
      .set('Content-Type', 'application/json');
    ...
    return this.httpClient.get<GetSkuProductResponse>(`${this.apiUrl}/getskuproduct`, { headers, params });
  }
