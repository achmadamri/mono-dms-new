...
public interface ViewOrderRepository extends JpaRepository<ViewOrder, Integer> {
	@Query("select t from ViewOrder t where t.tbuId = :tbuId and t.tboMarketId in :lstTboMarketId and t.tboMarketId like %:tboMarketId% and t.tboFrontliner like %:tboFrontliner% and t.tboBrand like %:tboBrand% and t.tboOrderNo like %:tboOrderNo% and t.tboCreateDate >= :startDate and t.tboCreateDate <= :endDate")
	List<ViewOrder> find(@Param("tbuId") Integer tbuId, @Param("lstTboMarketId") List<String> lstTboMarketId, @Param("tboBrand") String tboBrand, @Param("tboMarketId") String tboMarketId, @Param("tboFrontliner") String tboFrontliner, @Param("tboOrderNo") String tboOrderNo, @Param("startDate") Date startDate, @Param("endDate") Date endDate, Pageable pageable);
	
	@Query("select count(0) from ViewOrder t where t.tbuId = :tbuId and t.tboMarketId in :lstTboMarketId and t.tboMarketId like %:tboMarketId% and t.tboFrontliner like %:tboFrontliner% and t.tboBrand like %:tboBrand% and t.tboOrderNo like %:tboOrderNo% and t.tboCreateDate >= :startDate and t.tboCreateDate <= :endDate")
	Long count(@Param("tbuId") Integer tbuId, @Param("lstTboMarketId") List<String> lstTboMarketId, @Param("tboBrand") String tboBrand, @Param("tboMarketId") String tboMarketId, @Param("tboFrontliner") String tboFrontliner, @Param("tboOrderNo") String tboOrderNo, @Param("startDate") Date startDate, @Param("endDate") Date endDate);
